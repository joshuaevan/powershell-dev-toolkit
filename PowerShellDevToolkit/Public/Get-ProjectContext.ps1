function Get-ProjectContext {
    ...
    .DESCRIPTION
        ...
        project type detection, key files, dependencies, git status,
        and available scripts/commands.
    ...
    function Get-ContextProjectType {
    ...
    function Get-ContextDependencies {
        param([string]$ProjectPath)
    ...
    function Get-ContextAvailableScripts {
        param([string]$ProjectPath)
    ...
    function Get-ContextProjectStructure {
        param([string]$ProjectPath)
    ...
    $projectType = Get-ContextProjectType -ProjectPath $Path
    $context.type = $projectType.type
    $context.framework = $projectType.framework
    $context.dependencies = Get-ContextDependencies -ProjectPath $Path
    $context.scripts = Get-ContextAvailableScripts -ProjectPath $Path
    $context.git = Get-ContextGitInfo -ProjectPath $Path
    $context.structure = Get-ContextProjectStructure -ProjectPath $Path
...
"@
        ...
        if ($context.scripts.Count -gt 0) {
            ...
            foreach ($s in $context.scripts | Select-Object -First 10) {
                $output += "`n  - $($s.name)"
            }
        }
