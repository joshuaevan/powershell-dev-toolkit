...
.EXAMPLE
    ...
    useenv .env.local      # Load specific file
    useenv -Show           # Show current env vars
    useenv -List           # Preview what would be set
#>
...
if ($Show) {
    ...
    # Show commonly used dev variables
    $devVars = @(
...
# Unload previously loaded variables
if ($Unload) {
    if ($global:LoadedEnvVars -and $global:LoadedEnvVars.Count -gt 0) {
        foreach ($var in $global:LoadedEnvVars) {
...
# List mode - just show what would be set
if ($List) {
    Write-Host ""
...
# Store loaded vars for later unloading
